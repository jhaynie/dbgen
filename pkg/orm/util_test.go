...
func TestToString(t *testing.T) {
	...
	assert.Equal(ToString(sql.NullString{"", false}), "", "should have been empty string")
	assert.Equal(ToString(&sql.NullString{"", false}), "", "should have been empty string")
	assert.Equal(ToString(sql.NullString{"yes", true}), "yes", "should have been yes")
	assert.Equal(ToString(&sql.NullString{"yes", true}), "yes", "should have been yes")
	assert.Equal(ToString(123), "123", "should have been 123")
	...
	assert.Equal(ToString(sql.NullInt64{123, true}), "123", "should have been 123")
	assert.Equal(ToString(&sql.NullInt64{123, true}), "123", "should have been 123")
	assert.Equal(ToString(&sql.NullInt64{0, false}), "0", "should have been 0")
	assert.Equal(ToString(&sql.NullFloat64{0, false}), "0.000000", "should have been 0.000000")
	assert.Equal(ToString(&sql.NullFloat64{123, true}), "123.000000", "should have been 123.000000")
	assert.Equal(ToString(sql.NullFloat64{123, true}), "123.000000", "should have been 123.000000")
	assert.Equal(ToString(sql.NullBool{true, true}), "true", "should have been true")
	assert.Equal(ToString(&sql.NullBool{true, true}), "true", "should have been true")
	assert.Equal(ToString(sql.NullBool{false, true}), "false", "should have been false")
	assert.Equal(ToString(&sql.NullBool{false, true}), "false", "should have been false")
	tv := time.Now()
	tvs := tv.String()
	assert.Equal(ToString(tv), tvs, "should have been "+tvs)
	assert.Equal(ToString(mysql.NullTime{tv, true}), tvs, "should have been "+tvs)
}
...
func TestTimestamp(t *testing.T) {
	...
	dt := ToTimestamp(mysql.NullTime{time.Now(), true})
	assert.NotNil(dt)
	sdt := ToSQLDate(dt)
